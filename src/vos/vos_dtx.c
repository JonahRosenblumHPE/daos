...
static bool
vos_dtx_rec_is_invalid(struct umem_instance *umm, struct vos_dtx_blob_df *dbd,
		       struct vos_dtx_act_ent_df *dae_df, umem_off_t rec)
{
	...
	case DTX_RT_ILOG: {
		...
		if (!vos_ilog_is_valid(ilog, dae_df->dae_lid)) {
...
static bool
vos_dae_df_is_invalid(struct umem_instance *umm, struct vos_dtx_blob_df *dbd,
		      struct vos_dtx_act_ent_df *dae_df)
{
	umem_off_t *rec_df;
	int         cnt;
	...
	for (i = 0; i < cnt; i++) {
		if (vos_dtx_rec_is_invalid(umm, dbd, dae_df, dae_df->dae_rec_inline[i]))
			return true;
	}
	...
	if (!UMOFF_IS_NULL(dae_df->dae_rec_off)) {
		...
		rec_df = umem_off2ptr(umm, dae_df->dae_rec_off);
		for (i = 0; i < cnt; i++) {
			if (vos_dtx_rec_is_invalid(umm, dbd, dae_df, rec_df[i]))
				return true;
		}
	}
...
vos_dtx_act_reindex(struct vos_container *cont)
{
	...
	while (!UMOFF_IS_NULL(dbd_off)) {
		...
		for (i = 0; i < dbd->dbd_index; i++) {
			...
			if (unlikely(vos_dae_df_is_invalid(umm, dbd, dae_df))) {
